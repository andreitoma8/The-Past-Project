...
def main():
    ...
    token = PastProject.deploy(account.address, {"from": account}, publish_source=True)
    nft = GemsCollection.deploy({"from": account}, publish_source=True)
    token_staking = StakingToken.deploy(
        token.address, nft.address, {"from": account}, publish_source=True
    )
